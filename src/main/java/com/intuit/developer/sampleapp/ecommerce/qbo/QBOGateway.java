...
public class QBOGateway {
    ...
    public SalesReceipt createSalesReceiptInQBO(ShoppingCart cart, String ccTransId) {
        ...
        txnTaxDetail.setTxnTaxCodeRef(findTaxCodeReference(dataService, "TaxCloud"));
        txnTaxDetail.setTotalTax(cart.getTaxAmount().getAmount());
	...
	private void appendLineItems(SalesReceipt salesReceipt, ShoppingCart cart) {
		...
		for (CartItem cartItem : cart.getCartItems()) {
			...
			// Set the unit price
			BigDecimal unitPrice = cartItem.getSalesItem().getUnitPrice().getAmount();
			lineDetail.setUnitPrice(unitPrice);
			line.setSalesItemLineDetail(lineDetail);
			line.setDescription(cartItem.getSalesItem().getName());

			// Set the line total
			line.setAmount(unitPrice.multiply(new BigDecimal(cartItem.getQuantity())));
