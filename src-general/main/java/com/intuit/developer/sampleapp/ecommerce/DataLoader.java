...
public class DataLoader {
    ...
    public static void initializeData(ConfigurableApplicationContext context) {
        if (oauthInfoNeeded(context)) {
            try {
                ...
                final File taxCloudFile = new File("taxcloud.json");
                final String taxCloudJSONStr = FileUtils.readFileToString(taxCloudFile);
                ObjectMapper taxCloudMapper = new ObjectMapper();
                final JsonNode taxCloudJSONNode = taxCloudMapper.readTree(taxCloudJSONStr);

                
                createAppInfo(jsonNode, taxCloudJSONNode, context);
    ...
    private static void createShoppingCart(Customer customer, ConfigurableApplicationContext springContext) {
        ...
        shoppingCart.setInstanceId(new Date().getTime());
    ...
    private static AppInfo createAppInfo(JsonNode jsonNode, JsonNode taxCloudJSONNode, ConfigurableApplicationContext context) {
        ...
        final JsonNode taxCloudJSONAppInfo = taxCloudJSONNode.get("taxCloudInfo");

        String taxCloudLoginID;
        String taxCloudAPIKey;
        
        try {
        	taxCloudLoginID = taxCloudJSONAppInfo.get("loginID").asText();
            taxCloudAPIKey = taxCloudJSONAppInfo.get("apiKey").asText();
        }
        catch (NullPointerException e) {
            RuntimeException rte = new RuntimeException("Exception occurred loading taxcloud.json verify that file contains valid json and that field names are correct.");
            rte.setStackTrace(e.getStackTrace());
            throw rte;
        }
        
        if (taxCloudLoginID.isEmpty()) {
            throw new RuntimeException("In 'taxcloud.json': 'loginID' property is empty");
        }
        if (taxCloudAPIKey.isEmpty()) {
            throw new RuntimeException("In 'taxcloud.json': 'apiKey' property is empty");
        }

        AppInfo appInfo = new AppInfo(appToken,consumerKey, consumerSecret, taxCloudLoginID, taxCloudAPIKey);
